...
class TestTransformerComponents(unittest.TestCase):
    ...
    def test_attention(self):
        ...
        self.assertTrue(torch.allclose(attn.sum(dim=-1), torch.ones(batch_size, seq_len)))
    ...
    def test_generator(self):
        ...
        self.assertTrue(torch.allclose(output.exp().sum(dim=-1), torch.ones(batch_size, seq_len)))
    ...
    def test_decoder_layer(self):
        ...
        ff = PositionwiseFeedForward(self.d_model, self.d_ff, self.dropout)
        decoder_layer = DecoderLayer(self.d_model, self_attn, src_attn, ff, self.dropout)
