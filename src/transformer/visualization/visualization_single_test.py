...
# Get the absolute path to the src directory (two levels up from current directory)
...
# New function to plot attention using matplotlib
...
# Function to visualize and save attention plots
def visualize_with_matplotlib(viz_data, src_text, tgt_text, output_prefix="attention"):
    """
    ...
    plt.savefig(f"{output_prefix}_encoder_self.png", dpi=200)
    plt.close(fig)
    ...
    fig.suptitle(f"Decoder Self-Attention: {tgt_text}", fontsize=16)
    ...
    plt.tight_layout(rect=[0, 0, 1, 0.96])  # Adjust for title
    plt.savefig(f"{output_prefix}_decoder_self.png", dpi=200)
    plt.close(fig)
    ...
    fig.suptitle(f"Decoder-Encoder Attention: {tgt_text} ← {src_text}", fontsize=16)
    ...
    plt.tight_layout(rect=[0, 0, 1, 0.96])  # Adjust for title
    plt.savefig(f"{output_prefix}_decoder_encoder.png", dpi=200)
    plt.close(fig)

    print(f"Attention visualizations saved as:")
    print(f"- {output_prefix}_encoder_self.png")
    print(f"- {output_prefix}_decoder_self.png")
    print(f"- {output_prefix}_decoder_encoder.png")
...
def visualize_single_example(model_path, src_text, tgt_text, config):
    ...
    visualize_with_matplotlib(viz_data, src_text, tgt_text)
...
viz_data = visualize_single_example(
    model_path="./epoch_40_bs128_acc21_warm3000.pt",
    src_text="我長大後想當國王。",
